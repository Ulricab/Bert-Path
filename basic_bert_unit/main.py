...
def save_pairs(train_ill, test_ill):
    
    with open("/root/bert-path/ref_pairs", "w", encoding='utf-8') as f:
        for tpl in test_ill:
            tpl_str = [str(x) for x in tpl]
            f.write("\t".join(tpl_str) + "\n")
    
    with open("/root/bert-path/sup_pairs", "w", encoding='utf-8') as f:
        for tpl in train_ill:
            tpl_str = [str(x) for x in tpl]
            f.write("\t".join(tpl_str) + "\n")
...
def main():
    ...
    ent2data, path2data, rel_triples_1, rel_triples_2 = read_data()
    print("---------------------------------------")
    ...
    if RANDOM_DIVIDE_ILL:
        ...
        save_pairs(train_ill, test_ill)
        train_ill = random.sample(train_ill, int(len(train_ill) * TRAIN_SIZE))
    ...
    train(Model,Criterion,Optimizer,Train_gene,train_ill,test_ill,ent2data, path2data)
