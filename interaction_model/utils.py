...
# def acyclic_paths_from_triples_iter(triples):
#     """
#     find pairs of triples ([a,b,c], [d, e, f]) s.t. c == d
#     create a digraph whose nodes are triples and edges are those pairs
#     return iterator over simple paths between t1, t2 in nodes
#     """

#     digraph = nx.DiGraph()
#     digraph.add_nodes_from(triples)
#     digraph.add_edges_from(list(get_pairs_iter(triples)))

#     for t1 in triples:  # yield singles
#         yield [t1]

#     for t1, t2 in combinations(triples, 2):  # yield paths
#         yield from nx.all_simple_paths(digraph, t1, t2)

def path_ent_dict_gene(rel_triples,max_length,pad_id = None):
    """
    get one hop neighbor of entity
    return a dict, key = entity, value = (padding) neighbors of entity
    """
    path_ent_dict = dict()
    for h,r,t in rel_triples:
        if h not in path_ent_dict:
            path_ent_dict[h] = []
        if t not in path_ent_dict:
            path_ent_dict[t] = []

    for h,r,t in rel_triples:
        if h == t:
            continue
        path_ent_dict[h].append(t)
        # for h2,r,t2 in rel_triples:
        #     if h2 == t:
        #         path_ent_dict[h].append(t2)
        #         # for h3,r,t3 in rel_triples:
        #         #     if h3 == t2:
        #         #         path_ent_dict[h].append(t3)
        #         #         # for h4,r,t4 in rel_triples:
        #         #         #     if h4 == t3:
        #         #         #         path_ent_dict[h].append(t4)
        #         #         #         # for h5,r,t5 in rel_triples:
        #         #         #         #     if h5 == t4:
        #         #         #         #         path_ent_dict[h].append(t5)
        #         #         #         #         break
        #         #         #         break
        #         #         break
        #         break
        print(path_ent_dict[h])


    #In order to get the maximum number of neighbors randomly for each entity
    for e in path_ent_dict.keys():
        np.random.shuffle(path_ent_dict[e])
        np.random.shuffle(path_ent_dict[e])
        np.random.shuffle(path_ent_dict[e])

    for e in path_ent_dict.keys():
        path_ent_dict[e] = path_ent_dict[e][:max_length]

    if pad_id != None:
        for e in path_ent_dict.keys():
            pad_list = [pad_id] * (max_length - len(path_ent_dict[e]))
            path_ent_dict[e] = path_ent_dict[e] + pad_list
    return path_ent_dict
