...
def main():
    ...
    path_emb = pickle.load(open(PATH_EMB_PATH,"rb"))
    print("read path embedding shape:", np.array(path_emb).shape)

    #read gcn embedding
    gcn_emb = pickle.load(open(ENT_GCN_EMB_PATH, "rb"))
    print("read gcn embedding shape:", np.array(gcn_emb).shape)
    ...
    neigh_dict = neigh_ent_dict_gene(rel_triples,max_length=ENTITY_NEIGH_MAX_NUM,
                                     pad_id=ent_pad_id)
    path_dict = path_ent_dict_gene(rel_triples,max_length=ENTITY_NEIGH_MAX_NUM,
                                     pad_id=ent_pad_id)
    ...
    entity_pairs_1,neighViewInterF = neighborView_interaction_F_gene(entity_pairs,ent_emb,neigh_dict,ent_pad_id,
                                  kernel_num=KERNEL_NUM,cuda_num=cuda_num,batch_size=512)
    
    entity_pairs_3,pathViewInterF = neighborView_interaction_F_gene(entity_pairs,ent_emb,path_dict,ent_pad_id,
                                kernel_num=KERNEL_NUM,cuda_num=cuda_num,batch_size=512)
    ...
    entity_pairs_4,pathdesViewInterF = desornameView_interaction_F_gene(entity_pairs,path_emb,
                                                    cuda_num=cuda_num,batch_size=512)
    
    _,gcnViewInterF = desornameView_interaction_F_gene(entity_pairs,gcn_emb,
                                                    cuda_num=cuda_num,batch_size=512)
    ...
    #save
    pickle.dump(neighViewInterF,open(NEIGHBORVIEW_SIMILARITY_FEATURE_PATH_1,"wb"))
    pickle.dump(pathViewInterF, open(NEIGHBORVIEW_SIMILARITY_FEATURE_PATH_2,"wb"))
    pickle.dump(desViewInterF,open(DESVIEW_SIMILARITY_FEATURE_PATH,"wb"))
    pickle.dump(pathdesViewInterF,open(PATH_DESVIEW_SIMILARITY_FEATURE_PATH,"wb"))
    pickle.dump(gcnViewInterF,open(GCNVIEW_SIMILARITY_FEATURE_PATH,"wb"))
    print("save neighbor-view similarty feature in:",NEIGHBORVIEW_SIMILARITY_FEATURE_PATH_1)
    print("save path-view similarty feature in:",NEIGHBORVIEW_SIMILARITY_FEATURE_PATH_2)
