...
def ent2desTokens_generate(Tokenizer,des_dict_path,ent_list_1,ent_list_2,des_limit = DES_LIMIT_LENGTH - 2):
    ...
    for ent,ori_des in ori_des_dict.items():
        if ent not in ent_set_1 and ent not in ent_set_2:
            continue
        string = ori_des
        encode_indexs = Tokenizer.encode(string, truncation=True, max_length=des_limit, add_special_tokens=False)
        ent2desTokens[ent] = encode_indexs
    print("The num of entity with description:",len(ent2desTokens.keys()))
    return ent2desTokens

def path2Tokens_gene(Tokenizer,entid2path,ent_list,index2entity,
                                ent_name_max_length = 275 - 2):
    path2tokenids = dict()

    for ent_id in ent_list:
        ent = index2entity[ent_id]
        ancestors = entid2path[ent_id][0]
        lowers = entid2path[ent_id][1]
        # ent_name = get_name(ent)
        token_ids = Tokenizer.encode(ent, truncation=True, max_length=ent_name_max_length,
                                         add_special_tokens=False)
        ancestors_ids = Tokenizer.encode(ancestors, truncation=True, max_length=ent_name_max_length,
                                         add_special_tokens=False)
        lower_ids = Tokenizer.encode(lowers, truncation=True, max_length=ent_name_max_length,
                                         add_special_tokens=False)
        
        path2tokenids[ent_id] = (token_ids, ancestors_ids, lower_ids)
    return path2tokenids

def path2bert_input(ent_ids,Tokenizer,path2token_ids,des_max_length=275):
    ent2data = dict()
    pad_id = Tokenizer.pad_token_id

    for ent_id in ent_ids:
        ent2data[ent_id] = [[],[]]
        ent_token_ids = Tokenizer.build_inputs_with_special_tokens(path2token_ids[ent_id][1])
        ent_token_ids = ent_token_ids + path2token_ids[ent_id][0] + [Tokenizer.sep_token_id] + path2token_ids[ent_id][1] + [Tokenizer.sep_token_id]
...
def read_data(data_path = DATA_PATH,des_dict_path = DES_DICT_PATH):
    ...
    entid2path = read_id2path_tuple_file([data_path + "paths_1",data_path + "paths_2"])
    ...
    Tokenizer = BertTokenizer.from_pretrained('pubmed-bert-abstract')
    ...
    path2tokenids = path2Tokens_gene(Tokenizer, entid2path, entids, index2entity)
    ent2data = ent2bert_input(entids,Tokenizer,ent2tokenids)
    path2data = path2bert_input(entids,Tokenizer,path2tokenids)

    return ent_ill, train_ill, test_ill, index2rel, index2entity, rel2index, entity2index, ent2data, path2data, rel_triples_1, rel_triples_2
