...
def main():
    ...
    for eid in range(0, len(eid2data.keys()), batch_size): #eid == [0,n)
        ...
        for i in range(eid, min(eid + batch_size, len(eid2data.keys()))):
            ...
            path_token_input = path2data[i][0]
            path_mask_input = path2data[i][1]
            token_inputs.append(token_input)
            mask_inputs.append(mask_input)
            print(path_token_input)
            path_token_inputs.append(path_token_input)
            path_mask_inputs.append(path_mask_input)

        vec = Model(torch.LongTensor(token_inputs).cuda(cuda_num),
                    torch.FloatTensor(mask_inputs).cuda(cuda_num))
        path_vec = Model(torch.LongTensor(path_token_inputs).cuda(cuda_num),
                    torch.FloatTensor(path_mask_inputs).cuda(cuda_num))
        
        path_emb.extend(path_vec.detach().cpu().tolist())
    ...
    pickle.dump(path_emb, open(PATH_EMB_PATH, "wb"))
    print("save path embedding....")


    #Generate candidates(likely to be aligned) for entities in train_set/test_set
    #we apply interaction model to infer a matching score on candidates.CANDIDATE_NUM
